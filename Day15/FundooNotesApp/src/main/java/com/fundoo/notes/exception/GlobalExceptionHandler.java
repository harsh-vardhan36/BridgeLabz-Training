...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleNoteTitleAlreadyExists(NoteTitleAlreadyExistsException ex) {
        Map<String, Object> error = new HashMap<>();
        error.put("timestamp", LocalDateTime.now());
        error.put("status", HttpStatus.CONFLICT.value());
        error.put("error", "Duplicate Note Title");
        error.put("message", ex.getMessage());
        return new ResponseEntity<>(error, HttpStatus.CONFLICT);
    }

    // Handle Invalid Note State Transitions
    @ExceptionHandler(InvalidNoteStateTransitionException.class)
    public ResponseEntity<Map<String, Object>> handleInvalidNoteStateTransition(InvalidNoteStateTransitionException ex) {
        Map<String, Object> error = new HashMap<>();
        error.put("timestamp", LocalDateTime.now());
        error.put("status", HttpStatus.CONFLICT.value());
        error.put("error", "Invalid Note State Transition");
        error.put("message", ex.getMessage());
        return new ResponseEntity<>(error, HttpStatus.CONFLICT);
    }
